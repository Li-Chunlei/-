...
![1](https://cdn.luogu.com.cn/upload/image_hosting/0u7em9pi.png) 
...
int main(){
    ...
    for(int i = 0;i < m;i++){
        cin>>x>>y;
        //朝向内，向左数
        if(a[now].number==0 && x==0)
          now=(now + n - y) % n;
        //朝向内，向右数
        else if(a[now].number==0 && x==1)
          now=(now + y) % n;
        //朝向外，向左数
        else if(a[now].number==1 && x==0)
          now=(now + y) % n;
        //朝向外，向右数
        else if(a[now].number==1 && x==1)
          now=(now + n - y) % n;
    }

    cout<<a[now].profess<<endl;
    return 0;
}